...
function hungarianService() {
    ...
    for (const jobOffer of jobOffers) {
      ...
      for (const application of applications) {
        ...
        scores.push({
          ...
          value: (degreeScore * 0.4) + (skillScore * 0.2) + (workExperienceScore * 0.4),
        });
    ...
    for (let i=0; i<applicantIDs.length; i++) {
        for (let j=0; j<jobOfferIDs.length; j++) {
          for (let score of scores) {
            matrix[i][j] = Infinity;
            if (jobOfferIDs[j] === score.jobOfferId && applicantIDs[i] === score.applicantId) {
              if (score.value === 0) {
                matrix[i][j] = Infinity;
              } else {
  ...
  async function runAlgorithm(jobOffers) {
    ...
    let matrix = await generateHungarianMatrix(jobOfferIDs, applicantIDs, scores);
    ...
    for (match of hungarian_matrix) {
      ...
      let applicant = (await applicantsService.getApplicantById(applicantId)).toJSON();
      applicant.jobOffer = (await jobsService.getJobById(jobOfferId)).toJSON();
      matchings.push(applicant);
    }
    return matchings;
  }
